...
def get_forecast():
    """
    Fetch add parse the weather forecast from clearoutside.com
    and returns a string like:

    "This morning, no rain, with an average temperature of 20 degrees.\n
    This afternoon, no rain, with an average temperature of 23 degrees.\n
    This night, nothing to see, with an average temperature of 16 degrees.\n
    Tomorrow morning, no rain, with an average temperature of 16 degrees.\n"
    ...
    print("hour => condition total clouds temperature => precipitation")
    ...
    if count_temp != 0:
        ...
        string_report += "This morning, "
        if count_rain == 0:
            string_report += "no rain, "
        elif count_sun == 0:
            string_report += "rain, "
        else:
            string_report += str(count_sun)+" hours of sun and "+str(count_rain)+" hours of rain, "
        string_report += "with an average temperature of " + str(math.floor(avg_temp)) + " degrees.\n"
    ...
    if count_temp != 0:
        avg_temp = sum_temp / count_temp
        string_report += "This night, "
        if count_good == 0:
            string_report += "nothing to see, "
        elif count_good == count_temp:
            string_report += "no cloud, you have to look at the sky, "
        else:
            string_report += str(count_good)+" over "+str(count_temp)+" hours without clouds, "
        string_report += "with an average temperature of " + str(math.floor(avg_temp)) + " degrees.\n"
    ...
    if count_temp != 0:
        avg_temp = sum_temp / count_temp
        string_report += "Tomorrow morning, "
        if count_rain == 0:
            string_report += "no rain, "
        elif count_sun == 0:
            string_report += "rain, "
    ...
    if count_temp != 0:
        avg_temp = sum_temp / count_temp
        string_report += "Tomorrow afternoon, "
        if count_rain == 0:
            string_report += "no rain, "
        elif count_sun == 0:
            string_report += "rain, "
